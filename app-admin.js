...
// ==========================================
// TAB 3: SCHEDULE GENERATOR & MANUAL OVERRIDE
// ==========================================
...
async function generateDraft() {
  ...
  try {
    ...
    function canAssign(pubId, dateObj, dateString) {
      if ((assignedCounts[pubId] || 0) >= 5) return false; 
...
// Helper to check live rules when admin manually overrides
...
function openShiftEditModal(shiftIndex) {
  ...
  // 1. Render currently assigned people
  ...
  // 2. Populate Dropdown (At a glance availability)
  const select = document.getElementById('shift-add-select');
  let optionsHtml = `<option value="">Seleccionar publicador...</option>`;
  
  let availableHtml = `<optgroup label="✅ Disponibles para este horario">`;
  let unavailableHtml = `<optgroup label="⚠️ No Disponibles (Forzar Asignación)">`;
...
function manualAdd(shiftIndex) {
  ...
  if(shift.assigned.length >= shift.capacity) {
      if(!confirm("⚠️ El turno ya alcanzó su capacidad máxima. ¿Forzar adición de todos modos?")) return;
  }

  // Check Rules for warnings
  let trackers = recalculateTrackers();
  let warnings = [];
  
  if ((trackers.counts[pub.id] || 0) >= 5) warnings.push("Ya tiene 5 turnos este mes.");
  if (trackers.dates[pub.id] && trackers.dates[pub.id].has(shift.dateString)) warnings.push("Ya está asignado a otro turno este mismo día.");
  ...
  if(warnings.length > 0) {
      let msg = `⚠️ ADVERTENCIA: Reglas rotas para ${pub.firstName}\n` + warnings.map(w=>"- "+w).join("\n") + "\n\n¿Forzar asignación de todos modos?";
      if(!confirm(msg)) return;
  }

  shift.assigned.push(pub);
  renderPreviewTable();
  openShiftEditModal(shiftIndex); // Refresh modal
}
...
function manualRemove(shiftIndex, pubId) {
  ...
  // Check Hard Pair Warnings
  if(pub.hardPair && pub.partner && shift.assigned.find(p => p.id === pub.partner)) {
      if(!confirm(`⚠️ ADVERTENCIA: Estás quitando a ${pub.firstName}, separando una Pareja Estricta (Hard Pair). ¿Continuar de todos modos?`)) return;
  }
  let partnerOf = shift.assigned.find(p => p.hardPair && p.partner === pubId);
  if(partnerOf) {
      if(!confirm(`⚠️ ADVERTENCIA: Estás quitando a la pareja de ${partnerOf.firstName}, quien tiene la regla de pareja estricta. ¿Continuar?`)) return;
  }
    ...
    for (const shift of draftSchedule) {
      if (shift.assigned.length > 0) { 
        await db.collection('shifts').add({
          date: shift.dateString, location: shift.location, time: shift.time,
