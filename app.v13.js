...
function parseData(rows) {
  ...
  // Note: CSV does not support hyperlinks.

  // 1. Dynamic Admin Phone Search
  // Instead of hardcoding rows[0][6], we search the first few rows for a phone-like number.
  let foundPhone = null;
  let debugSource = "Default";

  const maxSearchRows = Math.min(rows.length, 5);
  for (let r = 0; r < maxSearchRows; r++) {
      const row = rows[r];
      for (let c = 0; c < row.length; c++) {
          const cell = getVal(row, c);
          const digits = cell.replace(/\D/g, '');
          
          // Pattern: At least 10 digits to be safe (Paraguay mobile is usually 595 + 9 digits = 12)
          // We avoid small numbers or 8-digit strings that might be dates (20260101) or times (18302030)
          // Cell G1 is "595983281197" -> 12 digits.
          if (digits.length >= 10 && !cell.toLowerCase().includes("fecha")) {
               foundPhone = digits;
               debugSource = `Row ${r}, Col ${c} (${cell})`;
               break; // Found it
          }
      }
      if (foundPhone) break;
  ...
  if (foundPhone) {
      ...
      console.log("Admin Phone loaded from sheet:", ADMIN_PHONE);
  } else {
      console.log("Admin Phone not found in first 5 rows, using default.");
  }

  // Store debug info for Admin Panel
  window.debugPhoneSource = debugSource;
  window.debugFirstRow = rows.length > 0 ? JSON.stringify(rows[0]) : "No rows";
  ...
  // Fallback if no specific manager column found
  ...
  if (panel.style.display === 'none') {
    ...
    if (password === 'ppam2026') {
      ...
      const content = `
          ...
          <small>Fuente: ${window.debugPhoneSource || 'Default'}</small><br>
          <details style="margin-top:5px; cursor:pointer;">
             <summary>Ver Fila 0 (Debug)</summary>
             <div style="font-family:monospace; font-size:0.7em; white-space:pre-wrap; background:#eee; padding:5px; border-radius:5px; margin-top:2px;">${window.debugFirstRow || 'N/A'}</div>
          </details>
      `;
