...
function parseData(rows) {
  ...
  // 1. Extract Admin Phone from Cell G1 (Row 0, Col 6)
  // G is the 7th letter, so index 6.
  const phoneCell = getVal(rows[0], 6);
  if (phoneCell && phoneCell.length > 5) { // Basic validation
      // Clean the number (remove spaces, parens, dashes)
      ADMIN_PHONE = phoneCell.replace(/\D/g, ''); 
...
function renderSchedule() {
  ...
  scheduleData.forEach(day => {
    ...
    visibleSlots.forEach(slot => {
      ...
      // Regex to match "8:00 – 10:00" or "8 a 10" or "18:30 a 20:30"
  ...
  if (panel.style.display === 'none') {
    ...
    if (password === 'ppam2026') {
      ...
      if (!adminInfo) {
          ...
          infoDiv.innerHTML = `<strong>Número Admin Actual:</strong> ${ADMIN_PHONE} <br> (Cargado desde celda G1)`;
          panel.appendChild(infoDiv);
      } else {
          adminInfo.innerHTML = `<strong>Número Admin Actual:</strong> ${ADMIN_PHONE} <br> (Cargado desde celda G1)`;
      }
    } else {
...
function generateWeeklyReport() {
  ...
  upcomingDays.forEach(day => {
    ...
    day.slots.forEach(slot => {
        // Filter out "Disponible" if you don't want to spam the group with empty slots?
        // Or keep them to encourage people? Let's keep valid names only for "Reminders".
        // The user said "remembering their shifts", implying assigned people.
